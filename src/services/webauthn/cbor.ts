...
function lerCabecalho(cursor: Cursor): Cabecalho {
  ...
  const primeiro = cursor.dados[cursor.posicao] as number
  cursor.posicao++

  const maior = primeiro >> 5
  const info = primeiro & 0x1f

  if (info < 24) return { ok: true, maior, argumento: info }
  ...
  if (info > 26) return { ok: false, motivo: 'inteiro_grande_demais' }

  const octetos = info === 24 ? 1 : info === 25 ? 2 : 4
  if (cursor.posicao + octetos > cursor.dados.length) return { ok: false, motivo: 'truncado' }

  let argumento = 0
  for (let i = 0; i < octetos; i++) {
    argumento = argumento * 256 + (cursor.dados[cursor.posicao + i] as number)
  }
